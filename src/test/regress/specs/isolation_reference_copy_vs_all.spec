...
step "s1-real-time-select" { SELECT * FROM reference_copy ORDER BY id, data; }
step "s1-task-tracker-select"
{
	SET citus.task_executor_type TO "task-tracker";
	SELECT * FROM reference_copy AS t1 JOIN reference_copy AS t2 ON t1.id = t2.int_data ORDER BY t1.id, t1.data, t2.id;
}
...
step "s1-ddl-rename-column" { ALTER TABLE reference_copy RENAME data TO new_data; }
...
step "s2-real-time-select" { SELECT * FROM reference_copy ORDER BY id, data; }
...
step "s2-ddl-rename-column" { ALTER TABLE reference_copy RENAME data TO new_data; }
